...
import { isAlreadyClosed, isRetriable } from "./errors.ts";
import {
  Deferred,
  deferred,
      ...
      .catch(async (error) => {
        if (
          ...
          ((
            ...
            !this.connection.isClosed // Make sure that the connection wasn't explicitly closed by the user before trying to reconnect.
          ) || isRetriable(error))
        ) {
