...
export async function subscribe(writer: BufWriter, reader: BufReader, ...channels: string[]) {
    const sub = new RedisSubscriptionImpl(writer, reader);
    await sub.subscribe(...channels);
    return sub;
}

export async function psubscribe(writer: BufWriter, reader: BufReader, ...patterns: string[]) {
    const sub = new RedisSubscriptionImpl(writer, reader);
